...
class CameraSettingActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        queryValueByDPID("105")?.let {
            viewBinding.btnPrivate.text = it.toString()
        }
        viewBinding.btnPrivate.setOnClickListener {
            publishDps(
                "105",
                !java.lang.Boolean.parseBoolean(viewBinding.btnPrivate.text.toString())
            )

            listenDPUpdate("105", object : DPCallback {
                override fun callback(obj: Any) {
                    viewBinding.btnPrivate.text = obj.toString()
                }
            })
        }
