...
object PackageVerifier {
    ...
    fun isValidPackage(context: Context, packageName: String): Boolean {
        ...
        try {
            ...
            logD("Hash for $packageName: $hash")
        } catch (e: Exception) {
            logD("Error getting hash for $packageName: $e")
            return false
        }
    ...
    private fun verifyHash(context: Context, packageName: String, hash: String): Boolean {
        ...
        if (!prefs.contains(packageName)) {
            logD("Creating intial hash for $packageName")
        ...
        if (hash != existingHash) {
            logD("hash mismatch for " + packageName + ": expected " + existingHash
                    + ", got " + hash)
            return false
        }
        return true
