...
public class ChatRowImage extends ChatRowFile{
    ...
    private void showImageView(final Uri thumbernailPath, final Uri localFullSizePath, final Message message) {
        ...
        if (bitmap != null) {
            ...
            Glide.with(context).load(bitmap).apply(RequestOptions.bitmapTransform(new RoundedCorners(5))).into(imageView);//四周都是圆角的圆角矩形图片。
        } else {
            // 图片的大小
...
//                        imageView.setImageBitmap(image);
                        Glide.with(context).load(image).apply(RequestOptions.bitmapTransform(new RoundedCorners(5))).into(imageView);//四周都是圆角的圆角矩形图片。
