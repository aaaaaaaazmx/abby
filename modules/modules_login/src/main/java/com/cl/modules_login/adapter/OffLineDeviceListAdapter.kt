...
import com.cl.modules_login.databinding.LoginDeviceListItemBinding
import com.cl.modules_login.databinding.LoginDeviceListTextItemBinding
import com.cl.modules_login.databinding.LoginPairListItemBinding
import com.cl.modules_login.databinding.LoginPairTentInnerItemBinding
...
class OffLineDeviceListAdapter(
    ...
    init {
        addItemType(ListDeviceBean.KEY_TYPE_TEXT, R.layout.login_device_list_text_item)// 文字描述
        addItemType(ListDeviceBean.KEY_TYPE_BOX, R.layout.login_device_list_item)  // 设备
        addItemType(ListDeviceBean.KEY_TYPE_PH, R.layout.login_pair_list_item)  // ph配件
        addItemType(ListDeviceBean.MONITOR_VIEW_OUT, R.layout.login_pair_tent_inner_item) // 带屏外部温湿度传感器
        addItemType(ListDeviceBean.KEY_MONITOR_OUT, R.layout.login_pair_tent_inner_item) // 不带屏外部温湿度传感器
    }
    ...
    override fun onBindViewHolder(holder: BaseViewHolder, position: Int) {
        ...
        when (holder.itemViewType) {
            ListDeviceBean.KEY_TYPE_TEXT -> {
                val binding = DataBindingUtil.bind<LoginDeviceListTextItemBinding>(holder.itemView)
                if (binding != null) {
                    // 设置数据
            ...
            ListDeviceBean.MONITOR_VIEW_OUT -> {
                val binding = DataBindingUtil.bind<LoginPairTentInnerItemBinding>(holder.itemView)
                if (binding != null) {
                    // 设置数据
                    binding.data = data[position]
                    binding.executePendingBindings()
            ...
            ListDeviceBean.KEY_TYPE_BOX -> {
                val binding = DataBindingUtil.bind<LoginDeviceListItemBinding>(holder.itemView)
                if (binding != null) {
                    // 设置数据
                    binding.data = data[position]
                    binding.executePendingBindings()
            ...
            ListDeviceBean.KEY_TYPE_PH -> {
                val binding = DataBindingUtil.bind<LoginPairListItemBinding>(holder.itemView)
                if (binding != null) {
                    // 设置数据
                    binding.data = data[position]
                    binding.executePendingBindings()
