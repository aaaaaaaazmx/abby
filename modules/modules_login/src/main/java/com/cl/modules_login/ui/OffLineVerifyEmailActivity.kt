...
import com.cl.modules_login.ui.VerifyEmailActivity.Companion
 ...
 */
@AndroidEntryPoint
class OffLineVerifyEmailActivity : BaseActivity<ActivityVerifyEmailBinding>(),
    ...
    private val sendPop by lazy {
        RetransmissionPop(context = this@OffLineVerifyEmailActivity, onAgainAction = {
    ...
    private val xPopup by lazy {
        XPopup.Builder(this@OffLineVerifyEmailActivity)
            .hasStatusBar(false)
    ...
    override fun observe() {
        mViewModel.apply {
             ...
             * 检查是否种植过
             */
            checkPlant.observe(this@OffLineVerifyEmailActivity, resourceObserver {
                ...
                success {
                    ...
                    kotlin.runCatching { // 初始化SDK
                        ...
                        data?.let { PlantCheckHelp().plantStatusCheck(this@OffLineVerifyEmailActivity, it) }
                    }
                    finish()
                }
            })

            registerLoginLiveData.observe(this@OffLineVerifyEmailActivity) {
                when (it) {
                    is Resource.Loading -> {
                        showProgressLoading()
                    }
                            ...
                            "sms" -> {
                                mViewModel.tuYaLoginForPhone(
                                    ...
                                    onRegisterReceiver = { devId ->
                                        val intent = Intent(this@OffLineVerifyEmailActivity, TuYaDeviceUpdateReceiver::class.java)
                                        startService(intent)
                                    },
                                    onError = { code, error ->
                                        hideProgressLoading()
             ...
             * 检查此邮箱是否已经注册过
             */
            isBindEmail.observe(this@OffLineVerifyEmailActivity, resourceObserver {
                success {
                    if (data == false) {
                        // 跳转到修改密码界面, 需要区分是注册用户还是忘记密码用户,接口不一样
                        val intent =
                            Intent(this@OffLineVerifyEmailActivity, SetPasswordActivity::class.java)
                        intent.putExtra(
             ...
             * 第三方绑定邮箱
             */
            bindEmail.observe(this@OffLineVerifyEmailActivity, resourceObserver {
                error { errorMsg, code ->
             ...
             *  验证验证码
             */
            isVerifySuccess.observe(this@OffLineVerifyEmailActivity) {
                when (it) {
                    is Resource.DataError -> {
                        hideProgressLoading()
    ...
    override fun initData() {
        ...
        binding.btnSuccess.setOnClickListener {
            ...
            // 跳转到修改密码界面, 需要区分是注册用户还是忘记密码用户,接口不一样
            val intent =
                Intent(this@OffLineVerifyEmailActivity, OffLineSetPasswordActivity::class.java)
            intent.putExtra(
                KEY_REGISTER_OR_FORGET_PASSWORD,
                isRegister
            )
            ...
            intent.putExtra(VerifyEmailActivity.KEY_EMAIL_NAME, emailName)
            intent.putExtra(VerifyEmailActivity.KEY_EMAIL_CODE, binding.codeView.code)
            startActivity(intent)
        }
