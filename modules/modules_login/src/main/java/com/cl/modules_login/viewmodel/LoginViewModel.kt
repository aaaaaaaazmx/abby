...
class LoginViewModel @Inject constructor(private val repository: RegisterLoginRepository, @ApplicationContext private val context: Context) :
    ...
    fun tuYaLogin(
        ...
        if (email.isNullOrEmpty()) {
            tuYaLoginForPhone(map, interComeUserId, userInfo, deviceId, code, userInfo?.phoneNumber.toString(), password, onSuccess, onError, onRegisterReceiver)
            return
        }
        // 登录InterCome
        InterComeHelp.INSTANCE.successfulLogin(
                                                        ...
                                                        kotlin.runCatching {
                                                            ...
                                                            homeBean.deviceList.firstOrNull { it.devId == deviceId }.apply {
                                                                if (this == null && userInfo?.spaceType == ListDeviceBean.KEY_SPACE_TYPE_BOX && userInfo?.deviceOnlineStatus == "1") {
                                                                    ToastUtil.shortShow(context.getString(com.cl.common_base.R.string.string_631))
                                                                } else {
                                                                    val deviceBean = this
                                                                    // 缓存用户第一个设备数据
                                                                    // 只取第一个
