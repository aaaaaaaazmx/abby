...
class SeedCheckViewModel @Inject constructor(private val repository: HomeRepository) :
    ViewModel() {
     ...
     * 育种检查
     */
    private val _checkPlant = MutableLiveData<Resource<SeedCheckNoTaprootData>>()
    val checkPlant: LiveData<Resource<SeedCheckNoTaprootData>> = _checkPlant
    fun checkPlant(plantId: String) = viewModelScope.launch {
        repository.seedCheckNoTaproot(plantId).map {
     ...
     * 更新植物信息
     */
    private val _updateGrowInfo = MutableLiveData<Resource<BaseBean>>()
    val updateGrowInfo: LiveData<Resource<BaseBean>> = _updateGrowInfo
    fun c(body: UpPlantInfoReq) = viewModelScope.launch {
        repository.updatePlantInfo(body).map {
            if (it.code != Constants.APP_SUCCESS) {
                Resource.DataError(
                    it.code, it.msg
                )
     ...
     * 检查是否种植过植物
     */
    private val _checkPlants = MutableLiveData<Resource<CheckPlantData>>()
    val checkPlants: LiveData<Resource<CheckPlantData>> = _checkPlants
    fun checkPlants() = viewModelScope.launch {
