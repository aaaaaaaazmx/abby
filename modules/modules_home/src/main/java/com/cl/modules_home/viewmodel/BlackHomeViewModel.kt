...
class BlackHomeViewModel @Inject constructor(private val repository: HomeRepository) : ViewModel() {
    ...
    */
     ...
     * 获取温度，如果有购买温湿度传感器的话。
     */
    fun temperatureConversionForTempForBlack(text: Int?): String {
        val isMetric = Prefs.getBoolean(Constants.My.KEY_MY_WEIGHT_UNIT, false)
        val data = _plantInfoLoop.value?.data ?: _plantInfo.value?.data
        // 默认为false
        if (isMetric) {
     ...
     * 获取温度，如果有购买温湿度传感器的话。
     */
    fun temperatureConversionForRoomTempForBlack(text: Int?): String {
        val isMetric = Prefs.getBoolean(Constants.My.KEY_MY_WEIGHT_UNIT, false)
        val data = _plantInfoLoop.value?.data ?: _plantInfo.value?.data
        val roomTemp = com.cl.common_base.ext.temperatureConversion(
            data?.envirVO?.roomTemp.safeToFloat(),
            isMetric
        )
        return if (roomTemp.isNotEmpty()) "(Room $roomTemp)" else ""
    }
    ...
    fun getRoomHumidityForBlack(humidity: Int?): String {
        val data = _plantInfoLoop.value?.data ?: _plantInfo.value?.data
        val roomHumidity = data?.envirVO?.roomHumiture
        return if (roomHumidity.isNullOrEmpty()) "$humidity" else "$humidity"
    }
