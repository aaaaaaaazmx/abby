...
class TentKitViewModel @Inject constructor(private val repository: HomeRepository) :
    ViewModel() {
     ...
     * 获取植物信息 多植物
     */
    private val _plantList = MutableLiveData<Resource<MutableList<MultiPlantListData>>>()
    val plantList: LiveData<Resource<MutableList<MultiPlantListData>>> = _plantList
    fun getPlantList(planId: String) = viewModelScope.launch {
        repository.multiplantList(planId)
            .map {
                if (it.code != Constants.APP_SUCCESS) {
                    Resource.DataError(
                        it.code,
    ...
    // 修改多植物
    private val _updateMultiPlant = MutableLiveData<Resource<BaseBean>>()
    val updateMultiPlant: LiveData<Resource<BaseBean>> = _updateMultiPlant
    fun updateMultiPlant(req: MutableList<MultiPlantListData>) = viewModelScope.launch {
        repository.updateMultiplant(req)
            .map {
                if (it.code != Constants.APP_SUCCESS) {
                    Resource.DataError(
                        it.code,
